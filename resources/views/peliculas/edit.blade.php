<x-layouts::app :title="__('Editar Película')">
    <div class="max-w-3xl mx-auto px-6 py-8">

        <div class="mb-8">
            <div class="flex items-center gap-3 mb-1">
                <div class="h-7 w-1 rounded-full bg-red-600"></div>
                <h1 class="text-3xl font-bold text-white tracking-tight">Editar Película</h1>
            </div>
            <p class="text-sm text-zinc-500 ml-4">{{ $pelicula->titulo }}</p>
        </div>
        ...
        <div class="bg-[#141414] border border-zinc-800 rounded-2xl overflow-hidden shadow-xl">
            ...
            <div class="p-6">
                <form method="POST" action="{{ route('admin.peliculas.update', $pelicula) }}"
                    ...
                    @csrf @method('PUT')
                    @include('peliculas._form', ['pelicula' => $pelicula])
                </form>
            </div>
        </div>
    </div>
